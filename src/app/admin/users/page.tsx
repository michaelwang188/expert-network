...
import { isSuperAdmin, getSponsorLabel } from "@/lib/roles"

function SponsorBadge({ tier }: { tier: { label: string; color: string } }) {
  return (
    <span style={{ background: tier.color + "18", color: tier.color, padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 500 }}>
      {tier.label}
    </span>
  )
}
...
export default async function AdminUsersPage() {
  ...
  const users = await prisma.user.findMany({ orderBy: { createdAt: "desc" }, select: { id: true, name: true, email: true, role: true, orgName: true, points: true, createdAt: true, sponsorTier: true } })
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {users.map(u => (
          <div key={u.id} style={{ background: "#fff", border: "0.5px solid #e0dfd8", borderRadius: 10, padding: 12, fontSize: 13, display: "flex", alignItems: "center", gap: 12 }}>
            ...
            {getSponsorLabel(u.sponsorTier) && <SponsorBadge tier={getSponsorLabel(u.sponsorTier)!} />}
