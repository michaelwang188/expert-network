...
HOME_HTTP=$(curl -sI --max-time 10 "$URL" 2>/dev/null | head -5)
if echo "$HOME_HTTP" | grep -qE "HTTP/(1\.1|2) (200|307)"; then
...
# 2. 登录页 → 200
if curl -sI --max-time 10 "$URL/login" 2>/dev/null | head -5 | grep -qE "HTTP/(1\.1|2) 200"; then
...
# 4. 专家库 → HTTP/2
if curl -sI --max-time 10 "$URL/experts" 2>/dev/null | head -5 | grep -qE "HTTP/(1\.1|2) 200"; then
...
# 5. 排行榜 → HTTP/2
if curl -sI --max-time 10 "$URL/leaderboard" 2>/dev/null | head -5 | grep -qE "HTTP/(1\.1|2) 200"; then
...
if curl -sI --max-time 10 "$URL/admin" 2>/dev/null | head -5 | grep -qE "HTTP/(1\.1|2)"; then
