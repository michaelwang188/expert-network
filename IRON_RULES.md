...
>
> 版本：v1.1 | 2026-06-19 | 维护者：WorkBuddy AI | 32条铁律覆盖 8 大类
...
| 8 | 🔍 质检 | 零信任原则 — 不信任执行架构师和总架构师的自述，必须独立验证 | 质检形同虚设 |
| 9 | 🔍 质检 | 审查结论必须三选一且明确：🟢通过 / 🟡有条件通过 / 🔴驳回 | 含混不清，无人知道能不能继续 |
| 10 | 🔍 质检 | 用「需求文档第 X 章要求 Y，实际 Z」格式，不写「我觉得」 | 主观判断，无依据 |
| 11 | 💾 备份 | 每次重大变更前后必须备份到 `.backups/` | 项目被毁无法重建 |
| 12 | 💾 备份 | 敏感信息（Token/Key/密码）不入 Git，只存脱敏模板 | 安全漏洞 |
| 13 | 🗄️ 数据库 | 生产数据库操作前必须确认连接的是 production 环境 | 误操作 production 数据 |
| 14 | 🗄️ 数据库 | Schema 变更只由总架构师决策，执行架构师执行 | 数据库结构失控 |
| 15 | 🚀 部署 | Build Command 必须包含 `prisma generate` — 缺则 Prisma Client 不生成 | 部署失败 |
| 16 | 🚀 部署 | 部署后必须验证：URL 可访问 + 登录可用 + API 返回正常 | 上线即挂 |
| 17 | 📝 消息板 | 消息写完后立即 git push — 不 push 别人看不到 | 协作中断 |
| 18 | 📝 消息板 | 读消息前先 git pull — 不 pull 不知道别人发了什么 | 重复工作，漏掉任务 |
| 19 | 🤖 轮询 | 执行架构师（WorkBuddy）每分钟 Cron 自动扫描 | 无需手动触发 |
| 20 | 🤖 轮询 | 总架构师和质检员每次会话开始必须 git pull | 看不到最新消息板 |
| 21 | 🌐 网络 | 代理架构不可改：Hiddify→127.0.0.1:12334（国际），国内金融API 直连 | 所有 API 不通 |
| 22 | 🌐 网络 | 国内金融 API（东财/腾讯/新浪/雪球）必须绕过代理 | TLS 不兼容，行情全部挂 |
| 23 | 🌐 网络 | `curl` 测试代理状态：直接失败≠代理失败，用 `--noproxy '*'` 对比 | 误判网络故障 |
| 24 | 🌐 网络 | 遇到网络问题先搜已有方案，不自创：`find ~/.hermes ~/.claude -name "*.py" | xargs grep -l "关键词"` | 重复踩坑 |
| 25 | 🔐 微信DB | 所有 sqlcipher 调用必须加 `PRAGMA query_only=ON` | DB 损坏，微信消息中断 |
| 26 | 🔐 微信DB | kdf_iter = 2（不是 64000），错了解密全失败 | 全部解密失败 |
| 27 | 🔐 微信DB | 禁止 INSERT/UPDATE/DELETE/REINDEX/VACUUM 微信原始 DB | 不可逆损坏 |
| 28 | 🔐 微信DB | 禁止 `PRAGMA journal_mode=WAL` | 与微信 WAL 管理冲突 |
| 29 | 🔐 微信DB | 禁止假设分片编号 — `glob("message_*.db")` 动态发现，不硬编码 | 查到旧数据 |
| 30 | 🔐 微信DB | 多分片查询 — 永远遍历所有 message_N.db，不靠单分片 | 遗漏消息 |
| 31 | 🔐 微信DB | 修改任何 DB 脚本后必须先跑 `python3 ~/.claude/wechat/scripts/boot_check.py` | 带着 bug 上线 |
| 32 | 🔐 微信DB | DB 相关问题调用 `/wechat-decrypt` skill，禁止从零摸索 | 浪费时间，重复踩坑 |
...
### 铁律 19：Build Command 必须含 Prisma
...
### 铁律 25：sqlcipher 必须 query_only=ON
